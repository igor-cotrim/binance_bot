# 🤖 Binance Listings Bot

A simple Python bot that monitors **newly listed trading pairs on Binance** and sends real-time notifications to your **Telegram** account.

## 🚀 Features

- Periodically fetches the list of symbols from Binance (`/api/v3/exchangeInfo`).
...
```bash
binance_bot/
├── bot.py          # Core bot logic (Binance + Telegram)
├── main.py         # Entry point to run the bot
├── config.py       # Configuration (token, chat id, interval)
├── requirements.txt
└── README.md
```
...
```
The id value (987654321 in this example) is your CHAT_ID. Put that into config.py.
